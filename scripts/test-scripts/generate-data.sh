...
check_validator() {
  ...
  for i in {1..5}; do
    if solana --url http://solana-validator:$VALIDATOR_PORT cluster-version &>/dev/null; then
      return 0
    fi
...
airdrop_if_needed() {
  ...
  for i in {1..3}; do
    balance=$(solana --url http://solana-validator:$VALIDATOR_PORT balance 2>/dev/null || echo "0")
    if (( $(echo "$balance < 1.0" | bc -l) )); then
      echo "Requesting airdrop of 2 SOL (attempt $i/3)..."
      if solana --url http://solana-validator:$VALIDATOR_PORT airdrop 2 &>/dev/null; then
...
generate_transactions() {
  ...
  for i in $(seq 1 $NUM_TRANSACTIONS); do
    ...
    tx_sig=$(solana --url http://solana-validator:$VALIDATOR_PORT transfer --allow-unfunded-recipient \
  ...
  solana --url http://solana-validator:$VALIDATOR_PORT balance $recipient
}
